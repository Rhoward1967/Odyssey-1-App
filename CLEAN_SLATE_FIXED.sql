-- FIX: Re-run CLEAN_SLATE with hardcoded user_id
-- Trigger blocks user_id updates, so we need to delete and re-insert
-- Using verified user_id: eca49ca9-b4ae-4e0e-b78a-fa1811024781
...
DO $$
DECLARE
    -- Hardcoded user_id (verified from auth.users)
    v_user_id uuid := 'eca49ca9-b4ae-4e0e-b78a-fa1811024781';
    v_c uuid;
BEGIN
    ...
    RAISE NOTICE '✅ COMPLETE: 14 customers, 21 schedules loaded with user_id: %', v_user_id;
END $$;
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE user_id IS NOT NULL) as with_user_id,
  '$' || (SUM(amount_cents) FILTER (WHERE frequency = 'monthly') / 100.0)::numeric(10,2) as monthly_total,
  '$' || (SUM(amount_cents) FILTER (WHERE frequency = 'annual') / 100.0)::numeric(10,2) as annual_total
