...
export async function createCalendar(calendar: Partial<Calendar>): Promise<Calendar> {
...
export async function getCalendarEvents(calendarId: string): Promise<CalendarEvent[]> {
  const { data, error } = await supabase.from('calendar_events').select('*').eq('calendarId', calendarId);
  if (error) throw error;
  return data || [];
}
