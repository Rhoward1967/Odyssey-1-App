...
import { CreditCard, Calendar, TrendingUp, AlertCircle, CheckCircle, Crown, Star, Zap } from 'lucide-react';
...
const TIER_CONFIG = {
  ...
  ultimate: { name: 'Ultimate', icon: Crown, color: 'bg-gold-500', price: '$299' }
};
...
export default function SubscriptionManagementPortal() {
  ...
  const handleUpgrade = async (tier: string) => {
    try {
      const { data } = await supabase.functions.invoke('ai-research-payment-processor', {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">Manage your AI research subscription and billing</p>
      </div>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-2">{tierConfig?.price}/month</p>
            </div>
            {subscription?.status === 'active' && (
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {Object.entries(TIER_CONFIG).map(([tier, config]) => {
          ...
          return (
            <Card key={tier} className={isCurrent ? 'ring-2 ring-purple-500' : ''}>
              <CardContent className="p-6 text-center">
                <TierIcon className="h-8 w-8 mx-auto mb-2 text-gray-600" />
                <h3 className="font-semibold">{config.name}</h3>
                <p className="text-2xl font-bold text-purple-600">{config.price}</p>
