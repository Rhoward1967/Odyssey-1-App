...
import SimpleBidderForm, { BidderInfo, ServiceSpecs } from '@/components/SimpleBidderForm';
...
const BiddingCalculator: React.FC = () => {
  ...
  type DocumentStatus = 'approved' | 'pending';
  ...
  const handleFormSubmit = (bidderInfo: BidderInfo, serviceSpecs: ServiceSpecs) => {
    setBidderInfo(bidderInfo);
    setServiceSpecs(serviceSpecs);
    // Generate agreement document
    const agreement = generateAgreementDocument(bidderInfo, serviceSpecs);
    const newDoc = {
      id: Date.now().toString(),
      title: `Service Agreement - ${bidderInfo.name}`,
  ...
  const generateAgreementDocument = (bidder: BidderInfo, specs: ServiceSpecs) => {
    // If selectedSpecs is not present, fallback to services list
    const specsDetails = (Array.isArray((specs as any).selectedSpecs) ? (specs as any).selectedSpecs : []).map((spec: any) =>
      `${spec.name}: ${spec.description} (${spec.squareFootage} sq ft @ $${spec.pricePerSqft}/sq ft = $${(typeof spec.squareFootage === 'number' && typeof spec.pricePerSqft === 'number' && !isNaN(spec.squareFootage) && !isNaN(spec.pricePerSqft) ? spec.squareFootage * spec.pricePerSqft : 0).toFixed(2)})`
    ).join('\n');
...
${specsDetails || specs.services.join(', ')}
...
// Total Estimate: $${((typeof (specs as any).totalEstimate === 'number' && !isNaN((specs as any).totalEstimate)) ? (specs as any).totalEstimate : 0).toFixed(2)}
          ...
          <div className="lg:col-span-3">
            {!bidderInfo ? (
              ...
              <div className="space-y-8">
                {/* Pass frequency string directly to OdysseyPricingEngine */}
                <OdysseyPricingEngine
                  specs={serviceSpecs!.services.join('; ')}
