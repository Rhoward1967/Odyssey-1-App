...
CREATE POLICY "System insert sessions" 
  ...
  TO authenticated
  WITH CHECK (true); -- System can insert any session
...
BEGIN
  ...
  SELECT 
    COALESCE(
      (COUNT(*) FILTER (WHERE metric_name LIKE '%failure%' OR metric_name LIKE '%error%')::NUMERIC / 
       NULLIF(COUNT(*), 0)) * 100,
      0
    )
  INTO error_rate
  ...
  WHERE timestamp > NOW() - INTERVAL '1 hour'
    ...
    AND (metric_name LIKE '%duration%' OR metric_name LIKE '%response_time%');
