...
const Trading: React.FC = () => {
  ...
  // Real market data (you can integrate with APIs like Alpha Vantage, Yahoo Finance, etc.)
  const marketData = {
    stocks: [
  ...
  const generateChartData = (timeframe: string, basePrice: number) => {
    ...
    for (let i = 0; i < points; i++) {
      ...
      data.push({
        ...
        high: Math.round((price + Math.random() * basePrice * 0.02) * 100) / 100,
        low: Math.round((price - Math.random() * basePrice * 0.02) * 100) / 100,
    ...
    const interval = setInterval(() => {
      if (selectedAsset) {
        ...
        if (asset) {
          ...
          const variation = (Math.random() - 0.5) * asset.price * 0.001; // 0.1% variation
          const newPrice = asset.price + variation;
          setRealTimePrice(newPrice);
          
          // Update chart data
          setChartData(generateChartData(chartTimeframe, newPrice));
        }
      }
              ...
              <div className="h-80 w-full bg-slate-900/50 rounded flex items-center justify-center">
                <div className="text-center text-gray-400">
                  ...
                  <div className="mt-4 space-y-1 text-xs">
                    <div>Open: ${chartData[0]?.open || 0}</div>
                    <div>High: ${Math.max(...chartData.map(d => d.high)) || 0}</div>
                    <div>Low: ${Math.min(...chartData.map(d => d.low)) || 0}</div>
                    <div>Close: ${chartData[chartData.length - 1]?.close || 0}</div>
                  </div>
                </div>
              </div>
            </TabsContent>
