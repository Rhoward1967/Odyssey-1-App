...
serve(async (req) => {
  try {
    ...
    if (!address || typeof address !== 'string') {
      return new Response(JSON.stringify({ validated: false, error: 'Missing or invalid address' }), { status: 400 });
    }
    if (!SMARTY_AUTH_ID || !SMARTY_AUTH_TOKEN) {
      return new Response(JSON.stringify({ validated: false, error: 'Smarty credentials not set' }), { status: 500 });
    }
    ...
    if (Array.isArray(data) && data.length > 0 && data[0].analysis && data[0].analysis.dpv_match_code === 'Y') {
      // Valid address
      ...
      return new Response(JSON.stringify({ validated: false, error: 'Address not valid', raw: data }), { status: 200 });
    }
  } catch (err) {
    return new Response(JSON.stringify({ validated: false, error: err.message || 'Unknown error' }), { status: 500 });
  }
});
