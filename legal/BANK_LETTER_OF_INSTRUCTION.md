# LETTER OF INSTRUCTION TO BANKING INSTITUTION
## Howard Jones Bloodline Ancestral Trust - Account Structure Requirements

**Date:** February 19, 2026 (Amended — Original: February 2, 2026)
**To:** [Bank Name - Truist Bank / Main Account Bank]
**From:** Howard Jones Bloodline Ancestral Trust, by and through Co-Trustees
**Re:** Account Opening and Non-Commingling Requirements Due to UCC-1 Secured Creditor Status

---

## I. INTRODUCTION AND PURPOSE

The **Howard Jones Bloodline Ancestral Trust** ("Trust") is opening multiple banking accounts for the Trust and its wholly-owned subsidiary, **Odyssey-1 AI LLC** ("LLC"). This Letter of Instruction provides critical information regarding the **mandatory separation of accounts** due to perfected UCC-1 security interests filed with the Georgia Superior Court Clerks Cooperative Authority (GSCCCA).

**Banking Institution is hereby notified that:**
1. The Trust holds a **first-priority UCC-1 lien** on ALL assets of Odyssey-1 AI LLC
2. Odyssey-1 AI LLC holds a **senior secured interest** in HJS Services LLC
3. **Non-commingling of accounts is legally required** to maintain perfected security interests
4. All revenue entering Odyssey-1 AI LLC accounts is **contractually obligated** to satisfy the Trust's senior debt first

---

## II. UCC-1 SECURED CREDITOR STRUCTURE - "TRIPLE-LOCK" ($1.05M Total)

### A. UCC-1 Filing #1: First Lock (Asset Protection)

**Filing Number:** 029-2026-000007  
**Filing ID:** 14472596  
**Filing Date:** January 7, 2026  
**County:** Clarke County, Georgia  
**Status:** ✅ ACCEPTED and RECORDED (Public Record)

**Parties:**
- **Secured Party:** Odyssey-1 AI LLC (owned 100% by Howard Jones Bloodline Ancestral Trust)
- **Debtor:** HJS Services LLC
- **Secured Amount:** $350,000
- **Collateral:** All assets, equipment, vehicles, client relationships, contracts, and goodwill of HJS Services LLC

**Purpose:** This filing establishes Odyssey-1 AI LLC's senior secured position over the legacy service business (HJS Services LLC). Equipment, fixtures, accounts receivable, and client contracts are secured collateral.

---

### B. UCC-1 Filing #2: Second Lock (Trust Creditor Position and Asset Protection)

**Reference Number:** 1d164254-5bbe-4525-b47e-84927bd367fb  
**Filing ID:** 14629748  
**Filing Date:** January 26, 2026  
**County:** Clarke County, Georgia  
**Status:** ✅ ACCEPTED and RECORDED (Public Record)

**Parties:**
- **Secured Party:** Howard Jones Bloodline Ancestral Trust
- **Debtor:** Odyssey-1 AI LLC (Georgia)
- **Collateral:** ALL assets, intellectual property, patents, copyrights, trademarks, accounts receivable, equipment, inventory, revenue streams, and future property of Odyssey-1 AI LLC

**Secured Asset Valuation:** $6,710,000,000 (Market-Leader Position, January 2026)
- **Conservative Valuation:** $366,000,000 (baseline defensibility across 29-patent portfolio)
- **Market-Based Valuation:** $950,000,000 (operational deployment + patent density + active revenue streams)
- **Optimistic Valuation:** $6,710,000,000 (market-leader positioning with 29-patent density at 3× Inflection AI levels)
- 29-patent intellectual property portfolio (including USPTO #63/913,134)
- R.O.M.A.N. 2.0 AI system (proprietary technology platform, $750M pre-prototype targeting $2.5B NIJ body armor market)
- 7-book copyrighted series (Case #1-15033889051)
- 3-tier subscription revenue platform ($99/$299/$999/month, active deployment generating $64K-$167K per client annually)
- Live production deployment (odyssey-1-app.vercel.app, deployed January 24-25, 2026, 17,000+ lines production code, 114 database tables)
- Operational proof: Currently billing enterprise clients with recurring monthly revenue

**Purpose:** This filing establishes the Trust as the **apex creditor** with first-priority perfected security interest in ALL assets of Odyssey-1 AI LLC. The Trust owns 100% of the LLC and simultaneously holds a senior lien on all LLC property.

---

### C. UCC-1 Filing #3: Third Lock (Personal/Labor Shield)

**Filing Number:** 029-2026-000102  
**Filing ID:** [Confirmation from Clarke County]  
**Filing Date:** February 5, 2026, 11:14:33 AM  
**County:** Clarke County, Georgia  
**Status:** ✅ ACCEPTED and RECORDED (Public Record)

**Parties:**
- **Secured Party:** Odyssey-1 AI LLC
- **Debtors:** Rickey A. Howard and Christla L. Howard (individuals)
- **Secured Amount:** $350,000
- **Collateral:** Personal assets, accounts, future earnings, labor income, and all private property of Rickey A. Howard and Christla L. Howard

**Purpose:** This filing protects Rickey Allan Howard and Christla L. Howard's personal assets, future earnings, and labor income from third-party creditor claims. If a personal creditor attempts to seize individual earnings or accounts, they encounter Odyssey-1 AI LLC's first-priority lien position.

---

## III. APEX CREDITOR HIERARCHY

```
┌──────────────────────────────────────────────────────┐
│   HOWARD JONES BLOODLINE ANCESTRAL TRUST (APEX)     │
│   - First-Priority UCC-1 Lien: $6.71B Assets        │
│   - Filing ID: 14629748 ($350K)                      │
└──────────────────────┬───────────────────────────────┘
                       │ Secured Creditor over
                       ▼
┌──────────────────────────────────────────────────────┐
│   ODYSSEY-1 AI LLC (Core Operating Entity)          │
│   - Holds TWO Senior Liens ($700K total):           │
│     ├─ Over HJS SERVICES LLC ($350K, #14472596)    │
│     └─ Over Rickey & Christla individuals ($350K,   │
│        #029-2026-000102, protects labor/earnings)   │
└──────────────────────┬───────────────────────────────┘
          │                            │
          │ Secured Interest           │ Secured Interest
          │ ($350K)                    │ ($350K)
          ▼                            ▼
    ┌──────────────┐          ┌────────────────────────┐
    │ HJS SERVICES │          │ Rickey A. Howard &     │
    │ LLC (Debtors)│          │ Christla L. Howard     │
    │ - Equipment  │          │ (Individuals - Debtors)│
    │ - Contracts  │          │ - Personal Assets      │
    │ - Fixtures   │          │ - Future Earnings      │
    └──────────────┘          └────────────────────────┘
```

**The Triple-Lock Architecture:** The Trust is the apex creditor holding a $350K lien on Odyssey-1 AI LLC. Odyssey-1 AI LLC simultaneously holds two $350K liens: one over the operational business (HJS Services LLC) and one over the individuals' personal assets and labor income (Rickey A. Howard & Christla L. Howard). Total combined senior priority: **$1,050,000.00**. Any creditor attempting to reach the $6.71B IP fortress must work backward through all three layers.

---

## IV. ACCOUNT STRUCTURE REQUIREMENTS (NON-COMMINGLING MANDATE)

Due to the UCC-1 secured creditor structure, the following accounts **MUST be maintained separately** to preserve perfected security interests and avoid legal commingling violations:

### Account 1: Trust Reserve Account

**Account Name:** Howard Jones Bloodline Ancestral Trust
**Account Type:** Trust Savings/Reserve Account
**Authorized Signatories:** Teara Howard (Co-Trustee), Joseph Lumpkin Jr. (Co-Trustee)
**NOT a Signatory:** Rickey Allan Howard (Grantor - maintains third-party separation)
**NOT a Signatory:** Christla Howard (Protected Lifetime Beneficiary - no banking authority)

**Purpose:**
- Receive distributions from Odyssey-1 AI LLC as secured creditor payments
- Hold Trust reserves separate from operating funds
- Maintain legal separation between Trust (creditor) and LLC (debtor)

**Funding Source:**
- Transfers from Odyssey-1 AI LLC Main Operating Account
- Investment income and asset distributions
- Proceeds from IP licensing or asset sales

**Legal Requirement:** This account MUST be separate from LLC operating accounts to maintain the Trust's secured creditor position. Commingling would jeopardize the perfected UCC-1 lien.

---

### Account 2: Expense Account (Odyssey-1 AI LLC Operations)

**Account Name:** Odyssey-1 AI LLC (Operational Expenses)
**Account Type:** Business Checking Account
**Authorized Signatories:** Teara Howard (Co-Trustee of 100% owner), Joseph Lumpkin Jr. (Co-Trustee of 100% owner)
**NOT a Signatory:** Rickey Allan Howard (maintains separation)
**NOT a Signatory:** Christla Howard (Protected Lifetime Beneficiary - no banking authority)

**Purpose:**
- Pay operational expenses (contractors, utilities, software subscriptions)
- Issue contractor payments (1099 workers - 5 active contractors)
- Business expense management (separate from revenue collection)

**Funding Source:**
- Transfers from Main Operating Account
- Allocated expense budget

**Legal Requirement:** Expense account must be separate to track operational costs and maintain accounting clarity for UCC-1 collateral reporting.

---

### Account 3: Main Operating Account (Odyssey-1 AI LLC Revenue Collection)

**Account Name:** Odyssey-1 AI LLC
**Account Type:** Business Checking Account
**Authorized Signatories:** Teara Howard (Co-Trustee of 100% owner), Joseph Lumpkin Jr. (Co-Trustee of 100% owner)
**NOT a Signatory:** Rickey Allan Howard (maintains separation)
**NOT a Signatory:** Christla Howard (Protected Lifetime Beneficiary - no banking authority)

**Purpose:**
- Receive Stripe deposits (subscription revenue: $99/$299/$999/month plans)
- Receive invoice payments (janitorial customer transitions from HJS)
- Primary revenue collection point

**Expected Monthly Deposits:**
- Year 1 (2026): $8,000-$12,000/month (conservative estimate)
- Year 2 (2027): $40,000-$60,000/month (growth projection)

**Fund Flow (MANDATORY):**
1. Stripe deposits revenue → Main Operating Account
2. Main Operating Account → Trust Reserve Account (senior creditor payments)
3. Main Operating Account → Expense Account (operational costs)

**Legal Requirement:** Revenue must flow through Main Operating Account FIRST, then be allocated to Trust (creditor) and Expense (operations) to satisfy UCC-1 security obligations.

---

## V. THIRD-PARTY WALL: TRUSTEE SIGNATURE AUTHORITY

**Critical Governance Structure:**

The Grantor (Rickey Allan Howard) has **intentionally stepped back** from all banking signature authority to perfect the "third-party wall" required for irrevocable trust asset protection. This ensures:

1. **Bank sees Trust as independent entity** (not Grantor's personal account)
2. **$366M+ assets are legally "locked"** in Trust jurisdiction
3. **Secured creditor position is maintained** (Grantor cannot access debtor's funds)
4. **Co-Trustees are the public face** of banking relationships

**Authorized Signatories (ALL Accounts):**
- **Teara Howard** (Co-Trustee) - Full banking authority
- **Joseph Lumpkin Jr.** (Co-Trustee) - Full banking authority

**NOT Authorized (Maintains Creditor Position):**
- **Rickey Allan Howard** (Grantor/Secured Creditor) - NO signature authority on bank accounts

**NOT Authorized (Protected Lifetime Beneficiary):**
- **Christla Howard** (Grantor's Spouse / Protected Beneficiary) - NO signature authority on bank accounts. Christla Howard holds irrevocable lifetime financial protections under Article III of the Trust instrument but carries no trustee or banking authority.

**Grantor's Role:**
- Managing Member of Odyssey-1 AI LLC (operational control)
- Stripe developer/architect (technical platform control)
- Strategic direction ("New Creation" business strategy)
- **NOT a bank signatory** (creditor separation perfected)

---

## VI. VERIFICATION AND DOCUMENTATION

**Banking Institution may verify the UCC-1 filings at:**

**GSCCCA Online Portal:** https://www.gsccca.org/search/  
**Search Type:** UCC Financing Statement Search  
**County:** Clarke County, Georgia

**Filing #029-2026-000007 (ID: 14472596):**
- Secured Party: Odyssey-1 AI LLC
- Debtor: HJS Services LLC
- Filing Date: January 7, 2026
- Amount: $350,000

**Filing #14629748 (IP/Revenue):**
- Secured Party: Howard Jones Bloodline Ancestral Trust
- Debtor: Odyssey-1 AI LLC
- Filing Date: January 26, 2026
- Collateral: ALL intellectual property, patents, revenue ($6.71B valuation)

**Filing #029-2026-000102 (Personal/Labor Shield):**
- Secured Party: Odyssey-1 AI LLC
- Debtors: Rickey A. Howard and Christla L. Howard (individuals)
- Filing Date: February 5, 2026, 11:14:33 AM
- Amount: $350,000
- Collateral: Personal assets, accounts, future earnings

**Supporting Documents Provided:**
1. ✅ Trustee Certificate of Authority (HJFAT-2026-001) - Notarized
2. ✅ Trust Instrument (Howard Jones Bloodline Ancestral Trust)
3. ✅ UCC-1 Filing Confirmations (all three filings documented in Trust):
   - Filing #029-2026-000007 (January 7, 2026, HJS Services LLC lien)
   - Filing #14629748 (January 26, 2026, Odyssey-1 AI LLC/IP lien)
   - Filing #029-2026-000102 (February 5, 2026, Rickey & Christla personal shield)
4. ✅ Bank Presentation Package (Creditor Position Overview)
5. ✅ IP Valuation Report ($366M/$950M/$6.71B valuations)
6. ✅ LLC Articles of Organization (Georgia Secretary of State)

---

## VII. NON-COMMINGLING LEGAL MANDATE

**Banking Institution is hereby instructed:**

1. **DO NOT commingle** Trust Reserve Account with LLC Operating Accounts
2. **DO NOT allow** transfers between accounts without proper authorization from Co-Trustees
3. **DO recognize** the Trust's senior secured creditor status over all LLC funds
4. **DO maintain** separate account statements for Trust and LLC accounts
5. **DO NOT add** Rickey Allan Howard as a signatory (third-party wall requirement)

**Legal Consequences of Commingling:**
- Loss of perfected UCC-1 security interest priority
- Potential piercing of Trust asset protection
- Violation of secured creditor contractual obligations
- Tax complications (Trust vs LLC income attribution)

**Banking Institution's Duty:**
The bank is notified of the UCC-1 secured creditor structure and is expected to maintain account separation to preserve the legal hierarchy documented in the GSCCCA public filings.

---

## VIII. REVENUE FLOW AND CONTRACTUAL OBLIGATIONS

**Odyssey-1 AI LLC Revenue Sources:**

1. **Subscription Platform (Primary):**
   - 3-tier SaaS pricing: Basic ($99/mo), Professional ($299/mo), Enterprise ($999/mo)
   - Stripe payment processing (live production, deployed January 24-25, 2026)
   - Active client billing generating $64K-$167K per client annually
   - Expected Year 1 revenue: $100,000-$150,000
   - Expected Year 2 revenue: $500,000-$750,000

2. **Legacy Janitorial Contracts (Transition):**
   - 14 customers transitioning from HJS Services LLC to Odyssey-1 AI LLC billing (March 1, 2026)
   - Monthly recurring invoices for janitorial services
   - Customer welcome letters sent (notifications@odyssey-1.ai)

3. **Future IP Licensing:**
   - 29-patent portfolio available for licensing
   - R.O.M.A.N. 2.0 technology commercialization ($750M pre-prototype addressing $2.5B NIJ body armor market)
   - Potential enterprise AI platform licensing (Universal Math Engine, 17,000+ lines production code)

**Contractual Payment Priority (UCC-1 Secured Obligations):**

All revenue collected by Odyssey-1 AI LLC is **contractually obligated** to satisfy the following payment priority:

1. **First Priority:** Howard Jones Bloodline Ancestral Trust (UCC-1 secured creditor)
2. **Second Priority:** Operational expenses (contractors, software, utilities)
3. **Third Priority:** Growth investments and reserves

**Why Accounts Cannot Be Commingled:**
The Trust's senior lien on all LLC revenue means Trust payments must be tracked separately from operational expenses. The Main Operating Account receives revenue, then allocates funds according to the UCC-1 priority structure.

---

## IX. BANKING INSTRUCTIONS SUMMARY

### For Truist Bank (All 3 Accounts at Single Institution):

**Account 1: Trust Reserve Account**
- ✅ Account Name: Howard Jones Bloodline Ancestral Trust
- ✅ Authorized Signatories: Teara Howard and Joseph Lumpkin Jr. (Co-Trustees ONLY)
- ✅ Purpose: Trust asset management and creditor position maintenance
- ✅ Initial deposit: $__________ (optional)
- ✅ Account Type: Business Checking or Money Market

**Account 2: Expense Account**
- ✅ Account Name: Odyssey-1 AI LLC
- ✅ Authorized Signatories: Teara Howard and Joseph Lumpkin Jr. (as Co-Trustees of 100% owner)
- ✅ Purpose: Operational expenses, payroll, contractor payments
- ✅ Initial deposit: $__________ (optional)
- ✅ Account Type: Business Checking

**Account 3: Main Operating Account**
- ✅ Account Name: Odyssey-1 AI LLC
- ✅ Authorized Signatories: Teara Howard and Joseph Lumpkin Jr. (as Co-Trustees of 100% owner)
- ✅ Purpose: Revenue collection (Stripe deposits), invoice payments
- ✅ Initial deposit: $__________ (optional)
- ✅ Account Type: Business Checking
- ✅ Special Requirements: ACH/Wire transfer authority, Stripe payout connection

**Additional Truist Banking Setup:**
1. ✅ Enable **internal ACH transfers** between all 3 accounts (instant, no fees)
2. ✅ Provide **separate account statements** for each account
3. ✅ Set up **online banking access** for Teara Howard and Joseph Lumpkin Jr. on all 3 accounts
4. ✅ Issue **debit cards** for all 3 accounts (Teara Howard & Joseph Lumpkin Jr.)
5. ✅ Do **NOT add** Rickey Allan Howard as signatory on any account
6. ✅ Connect Main Operating Account to **Stripe payment processing**:
   - Stripe account: Live mode (sk_live_51S2w0SDPqeWRzwCX...)
   - Expected deposits: $8K-$12K/month Year 1, scaling to $40K-$60K/month Year 2

**Benefits of Single-Institution Banking:**
- Instant internal transfers (no external ACH delays)
- Single relationship manager
- Streamlined account management
- Lower fees for internal transfers
- Easier reconciliation and reporting

---

## X. TRUSTEE AUTHORIZATION CONFIRMATION

**This Letter of Instruction is authorized by:**

**Howard Jones Bloodline Ancestral Trust**
By and through Co-Trustees:

_________________________________
Teara Howard, Co-Trustee
Date: _______________

_________________________________
Joseph Lumpkin Jr., Co-Trustee
Date: _______________

**Grantor Acknowledgment (Non-Signatory):**

I, Rickey Allan Howard, as Grantor of the Howard Jones Bloodline Ancestral Trust, acknowledge and confirm:
1. I am NOT a signatory on any bank accounts opened for the Trust or Odyssey-1 AI LLC
2. The Co-Trustees have full authority to execute this Letter of Instruction
3. The account separation requirements are mandatory to preserve UCC-1 secured creditor status
4. I maintain my role as Managing Member of Odyssey-1 AI LLC and Stripe platform architect, but have NO banking signature authority

_________________________________  
Rickey Allan Howard, Grantor  
Date: _______________

---

## XI. BANKING INSTITUTION ACCEPTANCE

**By opening the requested accounts, [Bank Name] acknowledges:**
1. Receipt and review of this Letter of Instruction
2. Understanding of the UCC-1 secured creditor structure
3. Commitment to maintain account separation (non-commingling)
4. Recognition of Co-Trustees as exclusive authorized signatories
5. Awareness of Grantor's intentional exclusion from signature authority

**Bank Officer Signature:**

_________________________________  
[Bank Officer Name, Title]  
[Bank Name]  
Date: _______________

---

## XII. CONTACT INFORMATION

**Howard Jones Bloodline Ancestral Trust:**  
P.O. Box 80054  
Athens, GA 30608

**Co-Trustees:**
- Teara Howard: 678-292-3583 | [Teara's email]
- Joseph Lumpkin Jr.: 706-207-9869 | fbltransportllc742@gmail.com

**Protected Lifetime Beneficiary (NOT a banking contact):**
- Christla Howard — all financial obligations to her are handled directly by Trustees per Trust Article III

**Odyssey-1 AI LLC (Managing Member - Operational Contact):**  
Rickey Allan Howard  
Phone: 800-403-8492  
Email: generalmanager81@gmail.com

**Legal/Compliance Questions:**  
Refer to Triple-Lock UCC-1 filings:
- Filing #029-2026-000007 (ID: 14472596) - HJS Services LLC lien
- Filing #14629748 - Odyssey-1 AI LLC/IP lien
- Filing #029-2026-000102 - Rickey & Christla personal/labor shield

Trustee Certificate of Authority (HJFAT-2026-001)

---

**END OF LETTER OF INSTRUCTION**

**Document Status:** Ready for Co-Trustee signatures and bank presentation  
**Creation Date:** February 2, 2026  
**Purpose:** Ensure proper account structure based on UCC-1 "Double-Lock" secured creditor hierarchy  
**Strategic Note:** This letter protects the Trust's apex creditor position and ensures banks understand the non-commingling legal requirement.
