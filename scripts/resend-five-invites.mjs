/**
 * 🚀 RESEND INVITATIONS: First 5 Contractors
 * Purpose: Resend emails to contractors with existing tokens (after domain verification)
 * Usage: node scripts/resend-five-invites.mjs
 */
...
const contractorTokens = [
  {
    name: 'Rodney Deadwyler',
    email: 'rodneydeadwyler1970@gmail.com',
    token: '40007f40-96d2-461d-a63a-98e8b9f49ae0'
  },
  {
    name: 'Jerry Johnson',
    email: 'justj802@gmail.com',
    token: 'd15d503e-8a87-434d-92b2-4ea7b924869d'
  },
  {
    name: 'Robert Hale',
    email: 'toehale@bellsouth.net',
    token: 'af6e5081-e766-4bfd-b584-e1387b9c3693'
  },
  {
    name: 'Andre Foster',
    email: 'drezulufoster@gmail.com',
    token: '55e1b7aa-1080-4208-aea3-2be16b257fd9'
  },
  {
    name: 'Jeremy Walker',
    email: 'advancedcarpetcleaninginc1@gmail.com',
    token: 'fbe38b81-7dad-408e-b822-98cff1b88cb4'
  }
];

console.log('🚀 RESENDING CONTRACTOR INVITATIONS - VERIFIED DOMAIN\n');
console.log('═'.repeat(60));
console.log('From: Rickey Howard <onboarding@odyssey-1.ai>');
console.log('Domain: odyssey-1.ai (VERIFIED)');
console.log('═'.repeat(60));

const results = [];

for (const contractor of contractorTokens) {
  console.log(`\n📤 Processing: ${contractor.name}`);
  console.log(`   Email: ${contractor.email}`);
  
  // Add 1 second delay to avoid rate limiting
  if (contractorTokens.indexOf(contractor) > 0) {
    await new Promise(resolve => setTimeout(resolve, 1000));
  }
  
  try {
    const inviteUrl = `https://odyssey-1-app.vercel.app/onboarding/contractor/${contractor.token}`;
    ...
    const updateResponse = await fetch(`${SUPABASE_URL}/rest/v1/contractors?onboarding_token=eq.${contractor.token}`, {
      method: 'PATCH',
      headers: {
    ...
    if (!updateResponse.ok) {
      console.warn(`   ⚠️ Warning: Could not update tracking (email still sent)`);
    }
...
console.log(`✅ Successful: ${successful.length}/${contractorTokens.length}`);
console.log(`❌ Failed: ${failed.length}/${contractorTokens.length}\n`);
...
console.log('\n🎯 NEXT STEPS:');
console.log('   1. Contractors will receive professional branded emails');
console.log('   2. Monitor Compliance tab for Triple-Lock submissions');
console.log('   3. Approve submissions as they arrive');
console.log('\n🛰️ March 1st countdown: 28 days');
console.log('🛡️ Domain Authority: SOVEREIGN\n');
