#!/usr/bin/env node
/**
 * Send onboarding invitations to all 5 contractors
 */
...
console.log(`📧 Sending onboarding invitations to ${contractors.length} contractors\n`);
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  ...
  if (sendError) {
    console.error(`❌ ${contractor.full_name}: FAILED - ${sendError.message}`);
  } else {
    const emailId = data?.id || 'unknown';
    console.log(`✅ ${contractor.full_name} (${contractor.email})`);
...
console.log('✅ All contractor invitations sent!');
